...
st.set_page_config(page_title="Tablero Industrial", page_icon="🏭", layout="wide")
...
@st.cache_resource
def conectar_sheets():
    """Función para conectar con Google Sheets usando Secrets de Streamlit."""
...
# ==========================================
# INTERFAZ 1: MENÚ PRINCIPAL (TARJETAS)
# ==========================================
if st.session_state['pagina_actual'] == 'inicio':
    st.title("🏭 Panel de Control Industrial")
    st.write("Bienvenido. Selecciona un módulo para comenzar a trabajar:")
    
    st.markdown("<br>", unsafe_allow_html=True)
    
    # Creamos un diseño de columnas para simular tarjetas (cards)
    col1, col2, col3 = st.columns([1, 1, 1])
    
    with col1:
        # Diseño de la tarjeta usando un contenedor
        with st.container(border=True):
            st.markdown("### ⚙️ Producción")
            st.write("Extracción de datos CSV y carga automática a Google Sheets (Detallada, SEC y No Conformidades).")
            st.markdown("<br>", unsafe_allow_html=True)
            # Botón que cambia el estado a la interfaz de producción
            if st.button("Abrir Módulo", key="btn_produccion", use_container_width=True, type="primary"):
                cambiar_pagina('produccion')
                st.rerun()

    with col2:
        # Tarjeta vacía para futuros módulos
        with st.container(border=True):
            st.markdown("### 📊 Próximamente")
            st.write("Espacio reservado para futuros módulos (ej. Calidad, Mantenimiento, etc.).")
            st.markdown("<br>", unsafe_allow_html=True)
            st.button("No disponible", disabled=True, use_container_width=True)

# ==========================================
# INTERFAZ 2: MÓDULO DE PRODUCCIÓN
# ==========================================
elif st.session_state['pagina_actual'] == 'produccion':
    ...
    st.title("⚙️ Extractor de Datos de Producción")
    ...
    if uploaded_file is not None:
        ...
        if st.button("🚀 Actualizar Base de Datos en Google Sheets", type="primary"):
            with st.status("Actualizando Google Sheets...", expanded=True) as status:
                try:
                    ...
                    st.write("✅ Pestaña 'produccion detallada' al día.")
                    ...
                    st.write("✅ Pestaña 'produccion SEC' al día.")
